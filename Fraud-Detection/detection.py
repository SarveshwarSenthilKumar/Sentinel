...
def avg_path_length(sample_size: int) -> float:
    if sample_size <= 1:
        return 0.0
    return 2.0 * (math.log(sample_size - 1) + 0.5772156649) - (2.0 * (sample_size - 1) / sample_size)
...
class IsolationTree:
    ...
    def _build(self, rows: List[Sequence[float]], depth: int):
    ...
    def _walk(self, node, row: Sequence[float], depth: int) -> float:
        if node["leaf"]:
            return depth + avg_path_length(node["size"])
        branch = "left" if row[node["feature_idx"]] < node["split"] else "right"
        return self._walk(node[branch], row, depth + 1)


def isolation_forest_scores(rows: Sequence[Sequence[float]], tree_count: int = 80) -> List[float]:
    if not rows:
        return []
...
class RuleEngine:
    def evaluate(self, txn: Dict, state: Dict) -> List[RuleHit]:
        rules = [
            ("high_amount_vs_baseline", txn["amount_vs_sender_avg_30d"] >= 3.0, 0.15, "amount is more than 3x sender baseline"),
            ("new_device_high_amount", txn["is_new_device"] and txn["amount"] > 7000, 0.20, "high-value transaction from unseen device"),
            ("new_ip_new_country", txn["is_new_ip"] and txn["is_new_country"], 0.15, "transaction originated from unseen IP and country"),
            ("impossible_travel", txn["geo_velocity_kmh"] > 900, 0.25, "impossible travel pattern detected"),
            ("velocity_spike", txn["sender_txn_count_5m"] >= 5, 0.20, "high transaction burst within 5 minutes"),
            ("many_new_recipients", txn["sender_unique_receivers_24h"] >= 8, 0.15, "sender transacted with unusually many recipients"),
            ("shared_device", txn["accounts_per_device_24h"] >= 4, 0.20, "device linked to multiple accounts"),
            ("shared_ip", txn["accounts_per_ip_24h"] >= 5, 0.20, "IP linked to multiple accounts"),
            ("smurfing", txn["count_transfers_just_below_threshold_24h"] >= 3, 0.25, "multiple transfers just below reporting threshold"),
            ("dormant_activation", txn["dormant_days"] >= 90 and txn["sender_txn_count_1h"] >= 3, 0.15, "previously dormant account became suddenly active"),
            ("rapid_multi_hop", txn["hop_chain_length"] >= 3 and txn["time_to_cashout_sec"] <= 300, 0.30, "funds moved rapidly through multiple accounts"),
        ]
        return [RuleHit(name, triggered, points, reason) for name, triggered, points, reason in rules if triggered]
...
def engineer_transaction_features(transactions: Sequence[Dict]) -> List[Dict]:
    ...
    beneficiary_histories = defaultdict(list)
    tx_by_sender_receiver = defaultdict(list)
    last_sender_geo = {}
    ...
    for tx in sorted(transactions, key=lambda item: item["timestamp"]):
        ...
        sender_histories[sender] = [prev for prev in sender_histories[sender] if ts - parse_timestamp(prev["timestamp"]) <= timedelta(days=1)]
        sender_history = sender_histories[sender]
        sender_5m = [prev for prev in sender_history if ts - parse_timestamp(prev["timestamp"]) <= timedelta(minutes=5)]
        sender_1h = [prev for prev in sender_history if ts - parse_timestamp(prev["timestamp"]) <= timedelta(hours=1)]
        sender_24h = sender_history
        receiver_24h = receiver_histories[receiver]

        last_geo = last_sender_geo.get(sender)
        if last_geo:
            distance = geo_distance_km(last_geo["geo_lat"], last_geo["geo_lon"], item["geo_lat"], item["geo_lon"])
            gap_hours = max((ts - last_geo["timestamp"]).total_seconds() / 3600.0, 1 / 3600.0)
        ...
        item["amount_vs_sender_avg_30d"] = round(item["amount"] / max(item["sender_avg_amount_30d"], 1.0), 3)
        ...
        item["senders_to_receiver_24h"] = len({prev["sender_account"] for prev in receiver_24h} | {sender})
        item["count_transfers_just_below_threshold_24h"] = len(just_below)
        item["dormant_days"] = min(item["sender_account_age_days"], int(time_since_last / 86400.0))
        ...
        beneficiary_histories[item["beneficiary_id"]].append(item)
        tx_by_sender_receiver[(sender, receiver)].append(item)
        device_accounts[item["device_id"]].add(sender)
        ip_accounts[item["ip_address"]].add(sender)
        last_sender_geo[sender] = {"geo_lat": item["geo_lat"], "geo_lon": item["geo_lon"], "timestamp": ts}
...
def detect_fraud_rings(transactions: Sequence[Dict]) -> Tuple[List[RingAlert], Dict[str, Dict]]:
    ...
    account_signals = defaultdict(lambda: {
    ...
    def search(path_accounts: List[str], path_txs: List[Dict], start: str, current: str) -> None:
        ...
        for tx in outgoing.get(current, []):
            ...
            if nxt == start and len(path_accounts) >= 3:
                ...
                if avg_hop <= 120:
                    evidence.append(f"funds moved through {len(path_accounts)} accounts in {max(sum(hop_seconds), 1.0):.0f} seconds")
                ...
                alert = RingAlert(
                    ...
                    risk_score=round(clamp(0.6 + (0.18 if avg_hop <= 120 else 0.0) + (0.16 if total_amount >= 20000 else 0.0) + len(path_accounts) * 0.02), 2),
                    evidence=evidence,
                ...
                for account in path_accounts:
                    ...
                    signal["flow_velocity_score"] = max(signal["flow_velocity_score"], clamp((len(path_accounts) / 5.0) * min(300 / max(sum(hop_seconds), 1.0), 1.0)))
                    signal["network_evidence"].extend(evidence)
                    signal["cluster_id"] = cluster_id
                    signal["accounts_involved"].update(path_accounts)
                    signal["suspicious_funds_total"] = max(signal["suspicious_funds_total"], total_amount)
                continue
    ...
    for account, signal in account_signals.items():
        device_shared = max((len(accounts) for device, accounts in device_to_accounts.items() if account in accounts), default=1)
        ip_shared = max((len(accounts) for ip, accounts in ip_to_accounts.items() if account in accounts), default=1)
        signal["shared_infra_score"] = clamp(max(device_shared / 5.0, ip_shared / 5.0))
        if signal["shared_infra_score"] >= 0.8:
            signal["network_evidence"].append(f"{max(device_shared, ip_shared)} linked accounts share the same device or IP")
        ...
        if signal["fan_in_out_score"] >= 0.6:
            signal["network_evidence"].append("fan-in / fan-out transfer concentration detected")

        smurf_count = sum(1 for tx in incoming.get(account, []) + outgoing.get(account, []) if 9000 <= tx["amount"] < 10000)
        signal["smurfing_score"] = clamp(smurf_count / 4.0)
        if signal["smurfing_score"] >= 0.5:
            signal["network_evidence"].append("structuring pattern detected with repeated transfers just below threshold")

        cashout_hits = [tx for tx in outgoing.get(account, []) if tx["is_cashout_node"]] + [tx for tx in incoming.get(account, []) if tx["is_cashout_node"]]
        signal["cashout_score"] = clamp(len(cashout_hits) / 2.0)
        if cashout_hits:
            signal["network_evidence"].append("funds are converging on a likely cash-out node")
...
def build_network_risk(tx: Dict, account_signals: Dict[str, Dict]) -> Tuple[float, List[str], Optional[str], List[str], Optional[float]]:
    ...
    for signal in signals:
        ...
        if signal.get("suspicious_funds_total"):
            suspicious_funds_total = max(suspicious_funds_total or 0.0, signal["suspicious_funds_total"])
    evidence = list(dict.fromkeys(evidence))[:5]

    return round(clamp(network_risk), 2), evidence, cluster_id, sorted(accounts), suspicious_funds_total


def combine_scores(transaction_anomaly_score: float, rule_score: float, network_risk_score: float) -> float:
    final_risk = (
...
def build_explanation(
    ...
    suspicious_funds_total: Optional[float],
) -> str:
    facts = []
    if rule_reasons:
    ...
    accounts_text = f" Linked accounts: {', '.join(accounts_involved[:5])}." if accounts_involved else ""
    return (
        f"Risk {severity} at {final_risk:.2f}. Recommended action: {action}. "
        f"Transaction anomaly={transaction_anomaly_score:.2f}, rules={rule_score:.2f}, network={network_risk_score:.2f}. "
        f"Evidence: {summary}.{accounts_text}"
    )
